...
function getDayName(date) {
  const day = new Date(date).toUTCString().slice(0, 3);
  switch (day) {
    case 'Sun':
      return 'Sunday';
    case 'Mon':
...
function getNextFriday(date) {
  ...
  const nextFriday = newDate.setDate(newDate.getDate() + toFri);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dif = dateEndMs - dateStartMs;
  return dif / 24 / 3600 / 1000 + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const nearestMonday = start.getTime();
  const inputDate = date.getTime();
  return Math.ceil((inputDate - nearestMonday) / (7 * 24 * 60 * 60 * 1000));
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = currentDate.getMonth(); i < 12; i += 1) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    ...
    i <= endDate.getTime();
    i += (countOffDays + countWorkDays) * msPerDay
  ) {
    let tmp = 0;
    let offset = i;
    while (
